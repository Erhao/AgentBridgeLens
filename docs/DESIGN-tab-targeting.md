...
## 模型：tabId 寻址 + 固定目标作为默认
- 所有页面工具新增可选参数 `tabId`。
- 每次调用的 tab 解析优先级：**显式 `tabId` > 固定目标(若存活) > 当前激活 tab**。
- 多 tab：因为每个调用都能带 tabId，agent 同时握多个 tabId 就能并行操作多个页（CDP 也按 tabId 并行）。"固定目标"只是省略 tabId 时的默认值，给人用的便利。
